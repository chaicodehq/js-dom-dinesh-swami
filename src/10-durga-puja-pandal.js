...
export function updatePandalRating(element, newRating) {
  ...
  let oldRating = Number(element.dataset.rating);
...
export function filterPandalsByZone(container, zone) {
  ...
  let items = container.querySelectorAll(".pandal");
  let result = [];

  for (let i = 0; i < items.length; i++) {
    if (items[i].dataset.zone === zone) {
      result.push(items[i]);
    }
  }
...
export function getPandalsByBudgetRange(container, min, max) {
  ...
  for (let i = 0; i < items.length; i++) {
    let budget = Number(items[i].dataset.budget);
...
export function sortPandalsByRating(container) {
  ...
  let items = Array.from(container.querySelectorAll(".pandal"));

  items.sort(function (a, b) {
    let r1 = Number(a.dataset.rating);
    let r2 = Number(b.dataset.rating);
